...
async function insertTrigger(venue, targetRole, assetUrl) {
  const result = await pool.query(
    `INSERT INTO venue_triggers (venue, target_role, asset_url)
     VALUES ($1, $2, $3) RETURNING id;`,
    [venue, targetRole || null, assetUrl]
  );
  return result.rows[0].id;
}

async function getTriggersSince(venue, role, sinceId) {
  const result = await pool.query(
    `SELECT id, asset_url FROM venue_triggers
     WHERE venue = $1 AND id > $2 AND (target_role IS NULL OR target_role = $3)
     ORDER BY id ASC;`,
    [venue, sinceId, role]
  );
  return result.rows;
