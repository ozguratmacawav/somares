...
// Picks N random, minimum-spaced moments (seconds since start) within the
// asymmetric role-work window for the shared filler broadcast, each paired
// with which of the 4 filler clips to play — random from session to
// session, but identical for all four phones within one session (both the
// timing AND which clip, so everyone hears the exact same "hold" phrase at
// the exact same real-world instant).
function randomFillerSchedule(fillerCount) {
  const WINDOW_START = 480;   // 8:00 — after everyone's well into their own role work
  const WINDOW_END = 1080;    // 18:00 — leaves a clear runway before the 18:41 retreat cue
  const MIN_GAP = 90;
  const COUNT = 5;
  const times = [];
  let attempts = 0;
  ...
  return times.map((t) => t.toFixed(1) + ':' + Math.floor(Math.random() * n)).join(',');
}
