...
async function getVenueState(venue) {
  const result = await pool.query(
    `INSERT INTO venue_state (venue) VALUES ($1)
     ...
     RETURNING mood_key, started_at;`,
    [venue]
  );
  return { moodKey: result.rows[0].mood_key, startedAt: result.rows[0].started_at };
}
...
async function getAudioManifest(venue, role) {
  ...
  return {
    ...
    ambiences: ambiences.rows.map((r) => ({ url: r.url, volume: r.volume })),
...
module.exports = {
  ...
  startExperience, resetExperience
};
