...
// Full reset: clears the shared clock AND every registration/role
// assignment for the venue, so the next person to register starts a
// genuinely fresh group (not appended onto whatever was there before).
async function resetSession(venue) {
  ...
  await pool.query(`DELETE FROM registrations WHERE venue = $1;`, [venue]);
  await pool.query(`DELETE FROM venue_counters WHERE venue = $1;`, [venue]);
}
...
// Total rows for the venue, active or not — used to cap registration at
// GROUP_SIZE. Unlike countActiveRegistrations this isn't about "who's here
// right now", it's "has this group already filled up" — an inactive
// (disconnected) registrant still holds their role until an operator reset.
async function countAllRegistrations(venue) {
  const result = await pool.query(`SELECT COUNT(*) c FROM registrations WHERE venue = $1;`, [venue]);
  return Number(result.rows[0].c);
}
...
module.exports = {
  ...
  touchLastSeen, listPresence, deleteRegistration, countActiveRegistrations, countAllRegistrations
};
