...
async function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // mood_key used to default to 'calm', which meant every participant's
      // very first poll silently auto-applied a mood nobody had actually
      // triggered. Now it starts out NULL — nothing plays until a conductor
      // explicitly sets a mood — and Reset clears it back to NULL.
      await pool.query(`ALTER TABLE venue_state ALTER COLUMN mood_key DROP DEFAULT;`);
      await pool.query(`ALTER TABLE venue_state ALTER COLUMN mood_key DROP NOT NULL;`);
...
// Clears the synchronized-start flag AND the current mood, so the next
// "Start" begins from a genuinely blank slate — no leftover mood from a
// previous run auto-applying itself to a fresh audience.
async function resetExperience(venue) {
  await pool.query(`UPDATE venue_state SET started_at = NULL, mood_key = NULL WHERE venue = $1;`, [venue]);
}
